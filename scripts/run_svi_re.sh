...
#response=$(curl -w "%{http_code}"  --cacert ${cert_path}/ca-cert.pem --cert ${cert_path}/api-user.pem -v --request POST https://${owner_ip}:${owner_svc_port}/api/v1/owner/svi --header 'Content-Type: text/plain' --data-raw "[{\"filedesc\" : ${script_file_name_str}, \"resource\" : ${script_file_name_str}},{\"exec\" :[\"chmod\", \"777\",${script_file_name_str}]},{\"exec\" :[${script_file_name_str}, \"${tb_ip}\", \"${tb_port}\", \"${tb_token}\"]}]")
#response=$(curl -w "%{http_code}"  --cacert ${cert_path}/ca-cert.pem --cert ${cert_path}/api-user.pem -v --request POST https://${owner_ip}:${owner_svc_port}/api/v1/owner/svi --header 'Content-Type: text/plain' --data-raw "[{\"filedesc\" : ${script_file_name_str}, \"resource\" : ${script_file_name_str}},{\"filedesc\" : \"test.sh\", \"resource\" : \"test.sh\"} ,{\"exec_cb\" :[\"/bin/bash\",\"test.sh\"]},{\"fetch\" : "result.txt"}]")
response=$(curl -w "%{http_code}" --cacert ${cert_path}/ca-cert.pem --cert ${cert_path}/api-user.pem -v --request POST https://${owner_ip}:${owner_svc_port}/api/v1/owner/svi --header 'Content-Type: text/plain' --data-raw "[{\"filedesc\" : ${script_file_name_str}, \"resource\" : ${script_file_name_str}},{\"filedesc\" : \"test.sh\", \"resource\" : \"test.sh\"},{\"exec_cb\" :[\"/bin/bash\",\"test.sh\"]},{\"fetch\" : \"result.txt\"}]")
